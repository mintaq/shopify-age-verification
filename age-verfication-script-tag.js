...
var rootLinkAgeV_Server = "https://minh.omegatheme.com/age-verification";
...
(function () {
  ...
  var myAppJavaScript = function ($) {
    ...
    $.ajax({
      url: `${rootLinkAgeV_Server}/api/shops/public/user-settings/${omega_ageV_shopDomain}`,
      type: "GET",
      dataType: "json",
    }).done((result) => {
...
function setLocalStorage() {
  ...
  if (Array.isArray(popupDisplaySelected)) {
    ...
    if (popupDisplaySelected.length > 0) {
      ...
      localStorage.setItem("_otBlockLocations", []);
    }
  }
}
